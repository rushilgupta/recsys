...
public class SimpleItemItemModel implements Serializable {
    ...
    /**
     * get score for a neighbor of an item
     */
    double getScoreForNeighbor(long neighbor, long item) {
    	List<ScoredId> neighbors = neighborhoods.get(item);
    	
    	for(ScoredId scoredId: neighbors) {
    		if(scoredId.getId() == neighbor) {
    			return scoredId.getScore();
    		}
    	}
    	return 0;
    }
